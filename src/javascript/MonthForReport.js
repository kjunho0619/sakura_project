...
class InOutIncomeColor extends Component{
    render(){
        return(
            <div className="IncomeInOutCode-div2">
...
class InOutExpendColor extends Component{
    render(){
        return(
            <div className="ExpendInOutCode-div2">
                {this.props.inOutCode}
            </div>
        );
    }
...
class PriceIncomeColor extends Component{
    ...
    render(){
        return(
            <div className="ImcomePrice-div2">
...
class PriceExpendColor extends Component{
    ...
    render(){
        return(
            <div className="ExpendPrice-div2">
                {this.numberFormat(this.props.price)}円
            </div>
        );
    }
...
class DayForReportList extends Component{
    ...
    TitleDateDiv(ManagementForDay){
        ...
        if(this.state.listDate === month){
            this.state.flag = false;
        }else if(this.state.listDate !== month){
            this.state.listDate = month;
        ...
        if(this.state.flag === true){
            ...
            return(
                <div className="Title-date-div">
                    <div className="Title-year-div2">{year}年</div>
                    <div className="Title-month-div2">{month + 1}月</div>
            ...
            {Object.keys(this.props.IdCheck).map(id => {
                ...
                return(
                    <div className="ManagementForDay-div">
                        ...
                        <div className="ManagementForDay-view-div2" key={id} onClick={function(){                        
                            this.props.isModalInOut();
                            this.props.selectKey(id);
                            this.props.selectInfo(ManagementForDay);
                        }.bind(this)}>
                            <div className="Day-view-div">
                                {Number(String(ManagementForDay.Date).substr(8,2))}日
                            </div>
                            <div className="AssetsCode-div2">
...
DayIncomeExpend(data, monthCut){
  if(this.state.listDate !== monthCut){
      this.state.listDate = monthCut;
  ...
  for(var i = 0; i< this.state.valueCount; i++){
      ...
      if(data.UserID === this.state.sessionUser){
          if(yearCut === Number(year)){
            ...
            if(triger === 'total'){
                //month컷
                var monthCut = String(data.Date).substr(5,2);

                //console.log(monthCut);

                this.DayIncomeExpend(data, monthCut);
...
valueView(){
  this.valueReset();
  this.state.IdCheck = {};
  this.checkID(this.state.year, this.state.triger);
